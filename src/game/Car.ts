...
function buildMesh(spec: CarSpec, color: number): THREE.Group {
  const g   = new THREE.Group();
  const mat = new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.45, flatShading: true });
  g.add(new THREE.Mesh(new THREE.BoxGeometry(spec.width, 0.45, spec.length), mat));
  const nose = new THREE.Mesh(new THREE.ConeGeometry(spec.width * 0.7, 2.0, 4), mat);
  nose.geometry.rotateX(Math.PI / 2);
  nose.position.z = spec.length / 2 + 0.9;
  g.add(nose);
  const cockpit = new THREE.Mesh(new THREE.BoxGeometry(spec.width * 0.45, 0.38, spec.length * 0.25), mat);
  cockpit.position.y = 0.38;
  g.add(cockpit);
  return g;
}
...
export class Car {
  ...
  private _diffConfig: DifficultyConfig | null;

  constructor(
    scene: THREE.Scene, color: number, isPlayer: boolean, startT: number,
    spec: CarSpec, diffConfig: DifficultyConfig | null = null,
  ) {
    this.color       = color;
    this.isPlayer    = isPlayer;
    this.trackT      = startT;
    this.spec        = spec;
    this._diffConfig = diffConfig;
    this._aiBaseSpeed = diffConfig
      ? diffConfig.aiSpeedBase + Math.random() * diffConfig.aiSpeedVar
      : 0;
    this.mesh = buildMesh(spec, color);
    scene.add(this.mesh);
  }
  ...
  update(dt: number, input: CarInput | null, track: Track, allCars: Car[], playerCar?: Car) {
    ...
    if (this.respawning) {
      ...
      this.mesh.visible  = Math.floor(this.respawnTimer * 8) % 2 === 0;
    ...
    if (this.falling) {
      ...
      this.mesh.visible = this.fallDepth < 80;

      if (this.fallDepth > 50) {
        this.falling      = false;
        this.fallDepth    = 0;
        this.fallVel      = 0;
        this.lateral      = 0;
    ...
    if (this.isPlayer && input) {
      ...
      const accel  = SPEED_ACCEL * this.spec.accelMult;
      const brake  = SPEED_BRAKE;

      if (input.accel)    this.speed = Math.min(this.speed + accel * dt, spdMax);
      if (input.brake)    this.speed = Math.max(this.speed - brake  * dt, 0);
      ...
      if (input.mouseLatTarget !== null) {
        ...
        this.lateral += diff * Math.min(dt * 5 * this.spec.handleMult, 1);
      } else {
        const dir   = (input.right ? 1 : 0) - (input.left ? 1 : 0);
        const scale = Math.min(this.speed / SPEED_PLAYER_MAX, 1);
        this.lateral += dir * LAT_SPEED * this.spec.handleMult * scale * dt * MAX_LAT;
      }
    ...
    // ── Clamp + fall check ───────────────────────────────────────────────
    if (!this.isPlayer) {
      this.lateral = Math.max(-MAX_LAT * 0.9, Math.min(MAX_LAT * 0.9, this.lateral));
    } else {
      if (Math.abs(this.lateral) > FALL_TRIGGER) {
        this.falling = true;
        this.fallAxis.set(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).normalize();
        return;
      }
      this.lateral = Math.max(-MAX_LAT * 1.1, Math.min(MAX_LAT * 1.1, this.lateral));
    }
  ...
  private _aiStep(dt: number, track: Track, allCars: Car[], draftBoost: number, player?: Car) {
    const cfg   = this._diffConfig!;
    const spdMax = this._aiBaseSpeed * this.spec.topSpeedMult + draftBoost;
    this.speed  += (spdMax - this.speed) * Math.min(dt * 2, 1);
    this.speed   = Math.max(0, Math.min(spdMax, this.speed));

    // Racing line target (look ahead based on difficulty)
    ...
    for (const o of allCars) {
      ...
      if (Math.abs(delta) < 0.015) {
        ...
        if (Math.abs(latDiff) < MAX_LAT * 0.5) avoid += Math.sign(latDiff) * MAX_LAT * 0.4;
      }
    }

    // Blocking on hard/insane: move toward player's lateral
    if (cfg.aiBlock && player) {
      const behindDist = trackDelta(this.trackT, player.trackT);
      if (behindDist > 0 && behindDist < 0.025) {
        target = target * 0.4 + player.lateral * 0.6;
      }
    }
